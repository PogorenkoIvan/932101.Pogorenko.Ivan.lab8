...
function saveRaws(){
    ...
    const objs = con.querySelectorAll('.Object')
    for(obj of objs){
        let key = obj.querySelector('.key').value
        let val = obj.querySelector('.value').value
        rows[key] = Number(val)
    }
    const text = document.createElement('p')
    text.textContent = JSON.stringify(rows, null, 1)
    con.append(text)
}
